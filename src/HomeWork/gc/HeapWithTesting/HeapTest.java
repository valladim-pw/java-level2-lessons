...
public class HeapTest {
	static int maxSize = 0;//1932735283;//1024*1024*1024 * 1.8 = 1.8GB
	...
	//static boolean reverseSortBySize = false;
	//static boolean reverseSortBySizeTwoIndex = false;
	//static long allFreeCycles = 0;
	...
	public HeapTest(int repStrategy,
	                ...
	                int freeIteratesNum) {
		// В аргументах конструктора можно задать параметры цикла тестирования,
...
//			}
	...
	private static long[] performMemoryOperations(Heap.Strategy strategy) {
		...
		// в параметре конструктора класса TestHeap
		while ((maxSize - allocated) > minSize) { //50000
			long lstart, lstop;
  ...
  отсортированную по общему среднему времени выполнения алгоритма стратегии */
	...
	//	board += "count compact two index: " + Heap.countCompactTwoInd;
		board += "\ncompact two index sizes: " + Heap.compactSizes;
		board += "\ncount compact all: " + Heap.countAllCompact;
		board += "\ncompact all sizes: " + Heap.compactAllSizes;
		board += "\ncompact periodic sizes: " + Heap.compactPeriodiclSizes;
		board += "\ncount defrag: " + Heap.countCoeffDefrag;
		board += "\n\n";
		//allFreeCycles = 0;
...
//						false,
//						true).getLeaderBoard());
//		System.out.println(new HeapTest(1,
//						2428800,
//						10000,
//						30,
//						10,
