...
function loadFrame() {
	...
	for (let i = 0; i < head_anchors.length; i++) {		
		head_anchors[i].addEventListener("click", function() {			
			...
			if (!checkIdAvail(link_ids, link_id)) {
				...
				demo.insertAdjacentHTML("beforeend", "loader false<br>");
			} else {
				
				openLoader(true);
				demo.insertAdjacentHTML("beforeend", "loader true<br>");
			}				
	...
	for (let i = 0; i < cols.length; i++) {
		...
		if (col.hasAttribute("data-html-scroll")) {
			...
			if (res_top < 1 ) {
				closeLoader();
				setTimeout(removeLoaderOpacity, 2000);
				demo.insertAdjacentHTML("beforeend", "loader scroll close<br>");
			}							
		} else {
			if (img.complete) 
	...
	if (scroll_top == 0 && !checkIdAvail(link_ids, page_id)) {
		...
		if (imags[imags.length - 2].hasAttribute("data-load")) { // берем атрибут предпоследней картинки, учитывая, 		
																														// что последней может быть empty-img, которая загружается отдельно
			addLinkToIdsString(link_ids, page_id);
			
			closeLoader();
			setTimeout(removeLoaderOpacity, 2000);
			demo.insertAdjacentHTML("beforeend", "loader images close<br>");
		}		
	}
