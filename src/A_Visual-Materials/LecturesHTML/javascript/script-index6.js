...
function loadFrame() {
	...
	for (let i = 0; i < head_anchors.length; i++) {		
		head_anchors[i].addEventListener("click", function() {			
			...
			if (!checkIdAvail(link_ids, link_id)) {
				openLoader(false);
				demo.insertAdjacentHTML("beforeend", "<br>loader false ");//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			} else {
				openLoader(true);
				demo.insertAdjacentHTML("beforeend", "<br>loader true ");//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			}
	...
	let t1 = Date.now();//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	let sum = 0;
	let txt = loc_button.innerText;
	//txt = txt.substring(0, txt.indexOf("&"));
	demo.insertAdjacentHTML("beforeend", "<br>" + txt);//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	for (let i = 0; i < cols.length; i++) {
		...
		if (col.hasAttribute("data-html-scroll")) {
			...
			if (diff2 == html.scrollTop) {
				...
				demo.insertAdjacentHTML("beforeend", "<br>loader scroll close");//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			}							
		} else {
			...
			if (img.complete) {
				...
				let diff = t2 - t1;
				sum += diff;
				demo.insertAdjacentHTML("beforeend", "<br>img " + i + ": " + diff);//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			} 
						
		}		
	}
	demo.insertAdjacentHTML("beforeend", "<br>SUM " + sum);//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	if (!data_scroll)	{
		if (scroll_top == 0 && !checkIdAvail(link_ids, page_id)) {
			...
			if (imags[imags.length / 2].hasAttribute("data-load")) { // берем атрибут предпоследней картинки, учитывая, что последней может быть empty-img, которая загружается отдельно	
				...
				closeLoader();
				setTimeout(removeLoaderOpacity, 1000);
				demo.insertAdjacentHTML("beforeend", "<br>loader images close<br>");//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			}
		}
	} else
		data_scroll = false;	
