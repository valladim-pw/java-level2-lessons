...
<iframe id="frame" onload="disableWinOpen(); setControlButsPos(); loadFrame(); setTimeout(linkIndexClick, 50); setTimeout(clickHighlightMenuBut, 50);" 
*/

function disableWinOpen() {
	
	let p_click = frame.contentWindow.document.getElementsByTagName("P");
	
	if (window.innerWidth <= 1100) {
		
		for (let i = 0; i < p_click.length; i++) {
			
			if (p_click[i].hasAttribute("onclick"))
				p_click[i].removeAttribute("onclick");
		}
	}			
}
...
function changePageLinksTarget(id1, id2) {
	...
	for (let i = 0; i < cols.length; i++) {
		...
		removeClass(cols[i], "active-img");
		
		if (i >= id1 && i <= id2 && id2 != 0)	{
			addClass(cols[i], "active-img");
			//img_curr.style.borderTop = "15px solid #b4b4fc";	
			//img_curr.style.borderLeft = "10px solid #7575e1";
		}
		...
		if (window.innerWidth <= 500) {
			...
			removeClass(cols[i + 1], "active-img");
			
			if (i + 1 >= id1 && i + 1 <= id2 && id2 != 0)	{
				addClass(cols[i + 1], "active-img");
				//img_next.style.borderTop = "15px solid #b4b4fc";	
				//img_next.style.borderLeft = "10px solid #7575e1";
			}
