...
public class Heap {
	...
	static int countCompactTwoInd = 0;
	static List<Integer> compactSizes = new ArrayList<>();
	static int countAllCompact = 0;
	static List<Integer> compactAllSizes = new ArrayList<>();
	//static int countCompactPeriodic = 0;
	static List<Integer> compactPeriodiclSizes = new ArrayList<>();
	static List<Integer> bigSizes = new ArrayList<>();
	
	static int countCoeffDefrag = 0;
	
	static int countSortTwoIndex = 0;
	...
	public Heap(int maxHeapSize) {
		...
		//Arrays.fill(this.bytes, (byte)-1); // Для наглядности массив байт свободной кучи можно заполнить -1
	}
...
//		}
	...
	*/
	public int malloc(int size) {
		...
		byte[] block = new byte[size];// Массив байт, имитирующий выделяемый блок
		...
		for (int i = 0; i < freeBlocks.size(); i++) {
			...
			if (freeBlock.size >= size) {
				...
				setBytes(freeBlock.start, block);// Копируем массив байт выделяемого блока в массив кучи
	...
	public void free(int ptr) {
		...
		for (int i = 0; i < allocatedBlocks.size(); i++) {
			...
			if (allocatedBlock.start == ptr) {
				...
				//Arrays.fill(block, (byte)-1);// Для наглядности массив байт освобождаемого блока можно заполинить значениями -1
				setBytes(ptr, block);// Копируем массив байт освобождаемого блока в массив кучи
				return;
			}
		}
	...
	*/
	public void compact() {
		...
		if (destination < bytes.length) {
			...
			setBytes(destination, block);// Обновляем массив кучи
		}
	}
	...
	public int periodicCompact(int size) {
		...
		if (toCompact) {
			compact();
			compactPeriodiclSizes.add(size);
			possibleCompact = false;
		}
	...
	// от стратегии, которые определены в перечислении Strategy
