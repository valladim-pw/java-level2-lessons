...
$(document).ready(function() {
	...
	let butLoc = parent.document.getElementById("page-indexes");
	let icon = parent.document.getElementById("icon");	

	$(window).scroll(function(){
		
		if($(document).scrollTop() > 100){
			$(butWrap).addClass("fix");
			if(window.parent.innerWidth < 1400 && window.parent.innerWidth > 1200) {
				butWrap.style.left = (-100) + "px";
			}
			if(window.parent.innerWidth < 1100 && window.parent.innerWidth > 500) {
				$(butLoc).addClass("new-top");
				butLoc.style.top = (icon.clientHeight * 1.6) + "px";
			}				
		} else {
			$(butWrap).removeClass("fix");
			butWrap.style.left = 0 + "px";
			$(butLoc).removeClass("new-top");
			butLoc.removeAttribute("style");
		}
	});
	...
	$(window.parent).resize(function(){
		...
		if(window.parent.innerWidth < 1100 && window.parent.innerWidth > 500) {
			if($(butWrap).hasClass("fix") && !$(butLoc).hasClass("new-top")) 
				$(butLoc).addClass("new-top");
			if($(butLoc).hasClass("new-top"))
				butLoc.style.top = (icon.clientHeight * 1.6) + "px";
						
		} else if(window.parent.innerWidth >= 1100 || window.parent.innerWidth < 500) {
			if($(butLoc).hasClass("new-top")) {
				$(butLoc).removeClass("new-top");
				butLoc.removeAttribute("style");
			}	
