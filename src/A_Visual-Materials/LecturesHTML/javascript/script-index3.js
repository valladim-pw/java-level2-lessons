...
let link_repeat = document.getElementById("link-repeat");
...
function changeBackLink(elem, back_ref) {	
  ...
  let repeat_id;	
	...
	//link_back.setAttribute("data-back-idx1", back_id);
	
	if (back_ref.indexOf("#") != -1) {
		
		repeat_id = elem_href.substring(elem_href.indexOf("#") + 1);		
		addRepeatIndex(link_repeat, repeat_id);
...
function addRepeatIndex(elem, add_id) {	
...
function getRepeatIndex() {
	...
	if (link_repeat.getAttribute("data-repeat-idx") != null) {
		attr = link_repeat.getAttribute("data-repeat-idx");
		
		attr = attr.substring(0, attr.lastIndexOf(","));
		get = attr.substring(attr.lastIndexOf(",") + 1);		
		
	}
	link_repeat.setAttribute("data-repeat-idx", attr);
...
function addRepeatLink(elem, add_href) {
	...
	elem.innerText = txt;
	//demo.insertAdjacentHTML("beforeend", "<br>addRepeatLink");//!!!!!!!!!!!!!!!!!Demo
	return txt;
}
...
function getRepeatLink() {
	...
	let txt = link_repeat.innerText;	
	
	txt = txt.substring(0, txt.lastIndexOf("N"));	
	let get = txt.substring(txt.lastIndexOf("N"));
	
	link_index.setAttribute("href", get);
	//demo.insertAdjacentHTML("beforeend", "<br>getRepeatLink");//!!!!!!!!!!!!!!!!!Demo
	
	link_repeat.innerText = txt;
...
function clearRepeat() {	
  //demo.insertAdjacentHTML("beforeend", "<br>clearRepeatLink");//!!!!!!!!!!!!!!!!!Demo
	if (repeat_link) {
		
		link_repeat.innerText = "";
		link_repeat.removeAttribute("data-repeat-idx");
		//link_repeat.removeAttribute("data-repeat-scroll");
...
function linkIndexClick() {
  ...
  let index_href = "";
	...
	if (inn_link) {		
		...
		index_href = link_index.getAttribute("href");
		
		if (back_href.indexOf("#") != -1) {	
			
			demo.insertAdjacentHTML("beforeend", "<br>repeat index_href: " + index_href);//!!!!!!!!!!!!!!!!!Demo
			anchor = frame.contentWindow.document.querySelector(".inner-anchor[href='" + index_href + "']");			
			
			if (repeat_start)		
				back_id = index_href.substring(index_href.indexOf("#") + 1);			
			else			
		...
		if (repeat_link) {			
			repeat_href = getRepeatLink();			
			//repeat_id = getRepeatIndex();
			
			if (repeat_href != "") {
				
				link_back.setAttribute("href", repeat_href);
				//link_back.setAttribute("data-back-idx1", repeat_id);
				repeat_start = true;
...
function loadFrame() {
	...
	for (let i = 0; i < inner_anchors.length; i++) {
      ...
      let e = event.currentTarget;		
			...
			if (repeat_link) {					
				repeat_index = changeBackLink(e, ref_index);
				link_back.setAttribute("href", repeat_index);
				
				addRepeatLink(link_repeat, repeat_index);	
				
			} else {
				repeat_back = changeBackLink(e, ref_back);
				addRepeatLink(link_repeat, repeat_back);	
			}						
	...
	for (let i = 0; i < cols.length; i++) {
		...
		if (col.hasAttribute("data-html-scroll")) {
			...
			demo.insertAdjacentHTML("beforeend", "<br>1)scroll attr: " + scroll_top + ", scroll html: " + html.scrollTop + ", diff2: " + diff2);//!!!!!!!!!!!!!!!!!Demo
			
			if (diff2 == html.scrollTop) {
				demo.insertAdjacentHTML("beforeend", "<br>2)scroll attr: " + scroll_top + ", scroll html: " + html.scrollTop + ", diff2: " + diff2);//!!!!!!!!!!!!!!!!!Demo
				closeLoader();
	...
	if (!data_scroll)	{
		if (imags[imags.length - 2].hasAttribute("data-load")) { // берем атрибут предпоследней картинки, учитывая, что последней может быть empty-img, которая загружается отдельно		
		
			if (scroll_top == 0 && !checkIdAvail(link_ids, page_id))																											
				addLinkToIdsString(link_ids, page_id);
			demo.insertAdjacentHTML("beforeend", "<br>3)images scroll attr: " + scroll_top + ", scroll html: " + html.scrollTop);//!!!!!!!!!!!!!!!!!Demo
			closeLoader();
			setTimeout(removeLoaderOpacity, 1000);			
		}
	} else
